...
# java -jar $rootDir/target/ixa-pipe-dep-eu-2.0.0-exec.jar -b $baliabideak -c $rootDir/dependentziak.conll

# you can add "-c conll_out_file" parameter in order to get the output also in CONLL format; for example:
# java -jar $rootDir/target/ixa-pipe-dep-eu-2.0.0-exec.jar -b $baliabideak -c $rootDir/dependencies.conll